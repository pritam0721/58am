...
const Hero = ({ brandGreen, brandBgGreen, darkMode }) => {
  return (
    ...
    ${
      darkMode ? "bg-zinc-900 border-zinc-800" : "bg-zinc-100 border-slate-400"
    }`}
        >
          <span
            className={`w-2 h-2 rounded-full ${brandBgGreen} animate-pulse`}
          ></span>
          <span className="text-sm font-medium">
            Available for Offshore Direct Access
          </span>
        </div>
        <h1 className="text-5xl md:text-7xl font-extrabold mb-8 tracking-tight leading-tight">
          Build Your <span className={brandGreen}>Offshore Captive Team</span>
          <br />
          With Direct Access
        </h1>
        <p className="text-xl text-zinc-500 dark:text-zinc-400 max-w-2xl mx-auto mb-10">
          Scale your business with dedicated professionals in India. 70% cost
          reduction, 100% operational transparency.
        </p>
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
          <button
            className={`${brandBgGreen} text-black px-10 py-4 rounded-full font-bold text-lg flex items-center group`}
          >
            Get Started{" "}
          ...
          <button
            className={`px-10 py-4 rounded-full font-bold text-lg border transition-all duration-300
    ${
      darkMode
        ? "hover:bg-white hover:text-black border-slate-300 text-white "
        : "hover:bg-black hover:text-white border-slate-300 text-black"
    }
